...
export class XmlAdapter implements ListingIntegrationAdapter {
  source = "xml" as const;
  ...
  async pullListings(input: unknown): Promise<ExternalListing[]> {
    ...
    return rows.map((row: any) => ({
      ...
      images: Array.isArray(row.images?.image)
        ? row.images.image.map((v: unknown) => String(v))
        : row.images?.image
          ? [String(row.images.image)]
          : []
    }));
  }
}
